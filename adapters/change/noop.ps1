<# .SYNOPSIS
Contract entrypoint for the noop lab adapter.
.DESCRIPTION
...
try {
    $splat = @{ Adapter = 'noop'; Operation = $Operation; ChangeId = $ChangeId; Target = $Target; Parameters = (ConvertFrom-Json -InputObject $Parameters); UseCurrentCredentials = $UseCurrentCredentials; Local = $Local; TargetTransport = $TargetTransport; TargetPort = $TargetPort }
