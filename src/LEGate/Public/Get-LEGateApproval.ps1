function Get-LEGateApproval {
    ...
    if ($CapturePath) { Write-LEGateJson -Path $CapturePath -Value @{ reviews = $reviews; timeEvidence = $TimeEvidence; workflowRunId = $WorkflowRunId; observedAt = ConvertTo-LEGateTimestamp -Value (Get-LEGateUtcNow) } }
    ...
    if ($null -eq $TimeEvidence -or $TimeEvidence.kind -ne 'github-approval-time-evidence' -or
    ...
    return [pscustomobject]@{ approvedBy = $reviewer; approvedAt = $TimeEvidence.approvedAt; source = 'github-review-history'; workflowRunId = $WorkflowRunId; environment = $Environment; timeEvidence = $TimeEvidence }
}
