function Wait-LEGateRun {
    ...
    param(
        ...
        [ValidateRange(1, 3600)][int]$MaxWaitMinutes = 45,
    ...
    try {
        while ($true) {
            ...
            $run = Invoke-LEGateRequest -Session $Session -Method GET -Path ('/test-runs/' + $TestRunId) -Deadline $deadline
            Write-LEGateJson -Path (Join-Path -Path $folder -ChildPath 'run.json') -Value $run
            # Preserve a late response, but never accept it as an in-budget result.
            if ((Get-LEGateUtcNow) -ge $deadline) { $timedOut = $true; break }
        ...
        $timedOut = $true
        Write-LEGateJson -Path (Join-Path -Path $folder -ChildPath 'poll-error.json') -Value @{ code = 'run-timeout'; lastResponseAvailable = ($null -ne $run) }
    }
    catch {
        ...
        if ((Get-LEGateUtcNow) -ge $deadline) { $timedOut = $true }
        else { throw }
    }
    $output = [pscustomobject]@{ id = $TestRunId; state = $null; result = $null }
    if ($null -ne $run) { $output = $run.PSObject.Copy() }
