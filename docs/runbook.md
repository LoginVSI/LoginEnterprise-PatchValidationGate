...
2. Disable continuous scheduling in the LE UI or use `Stop-LEGateContinuousTest`, then wait until its active sessions have drained. The gate requires isEnabled=false and no active sessions for that continuous test. Keep unrelated tests/operators off the reserved target too.
