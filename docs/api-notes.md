...
| Start test | `PUT /tests/{testId}/start` body `StartRequest { comment, testRunName }` | `201 ObjectId { id }` = the test run id | 409 when the test can't be started (already running or disabled). Put the change id in `testRunName` and the policy hash in `comment`. |
| Existing runs (idempotency) | `GET /tests/{testId}/test-runs?count=20&orderBy=created&direction=desc` | `TestRunResultSet` | Check for a run whose `testRunName` matches the change id before starting a new one. |
...
| Continuous test handoff | `GET /tests?testType=continuousTest`, `PUT /tests/{testId}/start` | | Same start call; the continuous test must already exist. Part 3. |
